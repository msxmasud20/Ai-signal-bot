...
║      🚀 VERSION 19.0 - CHANNEL SELECT + MAX STATS          ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
    """
    print(banner)
...
max_win_streak = 0
max_loss_streak = 0
target_channel_id = CHAT_ID  # 🔥 ডিফল্ট চ্যানেল
pending_channel = None  # 🔥 চ্যানেল পরিবর্তনের জন্য
...
def get_admin_keyboard():
    return [
        ...
        [
            {"text": "📢 চ্যানেল সেট", "callback_data": "set_channel"},
            {"text": "🔙 ব্যাক", "callback_data": "back_to_main"}
        ]
    ]

# ============================================================
# 📤 সিগন্যাল ফাংশন - টার্গেট চ্যানেলে পাঠায়
# ============================================================
async def send_signal(prediction, previous_result=None):
    global engine, signal_count, total_loss, total_profit, consecutive_wins, consecutive_losses, max_win_streak, max_loss_streak, target_channel_id
...
{streak_text}
    ...
    # 🔥 টার্গেট চ্যানেলে পাঠান
    if signal_image:
        await send_telegram_photo(caption, signal_image, target_channel_id)
    else:
        await send_telegram_message(caption, target_channel_id)
    
    if is_jackpot and jackpot_image:
        jackpot_caption = f"🎰 *JACKPOT WINNER!*\n📌 Period: {prediction['period'][-6:]}\n🎯 {prediction['prediction']} + {prediction['number']} = JACKPOT!\n⏱️ {signal_time}"
        await send_telegram_photo(jackpot_caption, jackpot_image, target_channel_id)
...
async def check_result(prediction, actual_number):
    global engine, last_result, total_loss, total_profit, consecutive_wins, consecutive_losses, max_win_streak, max_loss_streak, target_channel_id
...
{streak_text}
    ...
    if result_image:
        await send_telegram_photo(caption, result_image, target_channel_id)
    else:
        await send_telegram_message(caption, target_channel_id)
...
async def handle_message(message):
    global is_running, engine, admin_session, max_win_streak, max_loss_streak, target_channel_id, pending_channel
    ...
    # 🔥 চ্যানেল সেট করার জন্য পেন্ডিং চেক
    if user_id in admin_session and admin_session[user_id].get('step') == 'awaiting_channel':
        try:
            channel_id = int(text.strip())
            target_channel_id = str(channel_id)
            admin_session[user_id]['step'] = 'authenticated'
            await send_telegram_message(
                f"✅ *চ্যানেল সফলভাবে সেট করা হয়েছে!*\n\n"
                f"📢 নতুন চ্যানেল আইডি: `{target_channel_id}`\n"
                f"📡 এখন থেকে এই চ্যানেলে সিগন্যাল পাঠানো হবে।",
                chat_id
            )
            await send_telegram_keyboard(
                "📊 *এডমিন প্যানেল*\n\nচ্যানেল সেট করা হয়েছে!",
                get_admin_keyboard(),
                chat_id
            )
        except:
...
async def handle_callback(callback):
    global is_running, engine, admin_session, image_manager, max_win_streak, max_loss_streak, target_channel_id, pending_channel
    ...
    # 🔥 চ্যানেল সেট করার অপশন
    if data == 'set_channel':
        if not is_admin:
            await answer_callback(callback_id, "⛔ Admin only!")
            return
        
        admin_session[user_id]['step'] = 'awaiting_channel'
        await edit_message_text(
            chat_id, message_id,
            f"📢 *চ্যানেল সেট করুন*\n\n"
            f"বর্তমান চ্যানেল: `{target_channel_id}`\n\n"
            f"দয়া করে নতুন চ্যানেলের আইডি দিন:",
            get_admin_keyboard()
        )
        await answer_callback(callback_id, "📢 চ্যানেল আইডি দিন...")
        return
...
# ============================================================
async def start_bot():
    global engine, is_running, image_manager, max_win_streak, max_loss_streak, target_channel_id
    ...
    logger.info("📢 CHANNEL SELECT ACTIVE")
